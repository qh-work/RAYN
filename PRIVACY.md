...
This statement describes the upstream RAYN project. Forks and redistributed builds may behave differently and must publish their own accurate disclosures.

## Data RAYN does not collect

The upstream application has no RAYN account system, advertising SDK, analytics SDK, telemetry endpoint, or user-profile service. The project maintainer does not receive a user's location history, favorites, or settings from the application.

## Location and network requests

- RAYN requests location permission only when the user enables current-location weather.
